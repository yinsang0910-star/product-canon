...
`product-canon` 是一个低频 Codex skill（技能）。它只在项目定调初始化，或用户明确要求重新归并项目核心愿景、需求和用户意志时运行。

它把已确认的产品决定和当时可验证的起点事实整理为固定五份核心文档。它不负责日常审计、整理、纠错、状态刷新、ROADMAP 维护、Spec 编写或源码实施。
...
| `CURRENT_STATE.md` | 建制当时已验证、未验证和阻塞的源码/运行事实及证据指针 |
...
| 场景 | 入口 | 结果 |
| --- | --- | --- |
| 用户初始化项目并确定产品方向，或产品类 `grilling` 已结束且用户确认共同理解 | 项目定调初始化（含 Grilling 交接） | 创建固定五文件并落入已确认决定 |
| 用户明确要求归纳重建当前项目的核心、愿景、需求或用户核心意志 | 显式归并重建 | 完整迁移有效决定并保留非核心材料 |
...
flowchart LR
    ...
    P --> A["ARCHITECTURE"]
    P --> C["ACCEPTANCE"]
    E["源码与运行证据 / Source and runtime evidence"] --> S["CURRENT_STATE"]
...
Speckit 只能为合格的 `APPROVED` 条目填写 `Spec` 路径并推进状态，不能改写已经批准的用户结果、边界或验收。
...
```text
使用 $product-canon，为这个项目定调并初始化固定五个产品核心文件；未知决定保持 UNKNOWN，不创建 Spec。
```

显式归并重建：

```text
使用 $product-canon，归并重建本项目的核心愿景、需求和用户意志；完整保留有效产品路线与非核心历史，不创建 Spec。
```
...
> 产品意志归 `PRODUCT`，系统责任归 `ARCHITECTURE`，完成证据归 `ACCEPTANCE`，完整路线与 Spec 交接归 `ROADMAP`，建制时的当前事实归 `CURRENT_STATE`。
...
`product-canon` is a low-frequency Codex skill. It runs only when a project is being initialized and given a durable product direction, or when the user explicitly asks to consolidate and re-found the project's core vision, requirements, and user intent.

It organizes confirmed product decisions and the verifiable starting truth into exactly five core documents. It does not perform routine audits, cleanup, corrections, status refreshes, ROADMAP maintenance, Spec authoring, or source implementation.
...
| `ARCHITECTURE.md` | System context, component responsibilities, single state/data authorities, critical paths, and failure invariants |
...
| `CURRENT_STATE.md` | Source/runtime facts that were verified, unverified, or blocked at the time the canon was established, with evidence pointers |
...
| --- | --- | --- |
| The user initializes a project and sets its product direction, or a product-focused `grilling` interview has ended with confirmed shared understanding | Project initialization, including a Grilling handoff | Create the fixed five documents and materialize the confirmed decisions |
| The user explicitly asks to consolidate/rebuild the project's core vision, requirements, or user intent | Explicit canon re-foundation | Preserve every still-valid decision and keep non-canon material |
...
- API references, ADRs, designs, research, and historical archives;
- tests, runtime evidence, release receipts, and migration notes.
...
```text
Use $product-canon to set this project's direction and initialize the fixed five core product documents. Keep unknown decisions as UNKNOWN and do not create a Spec.
```

Explicit canon re-foundation:

```text
Use $product-canon to consolidate and re-found this project's core vision, requirements, and user intent. Preserve the complete valid product route and non-canon history. Do not create a Spec.
```
...
> Product intent belongs in `PRODUCT`; system responsibility in `ARCHITECTURE`; completion evidence in `ACCEPTANCE`; the complete route and Spec handoff in `ROADMAP`; and the current starting truth in `CURRENT_STATE`.
