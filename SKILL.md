...
| `ROADMAP.md` | 用户批准的纵向交付结果、依赖、顺序、状态和 Spec 交接 |
...
- 用 `ROADMAP.md` 记录用户批准的交付顺序；不要在 `CURRENT_STATE.md` 建立第二份下一步清单。
...
| 哪些用户结果获准交付、先后依赖和 Spec 状态 | `ROADMAP.md` |
...
2. 只合并 `MERGE_CORE`；保持 `KEEP_NON_CANON` 不变。
3. 只有用户明确取代旧决定，或新位置已保留完全相同含义时，才删除重复声明。
4. 更新本次范围内的权威链接；未经授权，不删除、重命名、移动或覆盖旧文件。
...
`ROADMAP.md` 可以自由包含产品愿景、能力范围、阶段、说明和分组；不要强制所有项目采用同一套叙事章节，也不要为了适配 Spec 删除这些内容。
...
- 只有用户明确批准的条目才能进入 `APPROVED`；不要根据产品终局自动生成完整实施清单。
...
3. 每个迁移输入都有处理方式；`UNRESOLVED` 没有被静默丢弃或写成已取代。
4. ROADMAP 交接表的列名、状态值和 ID 唯一性符合契约；表外叙事不产生 Spec。
